...
export const api = {
  ...
  managerApprove: (token: string, id: string, remarks: string) =>
    request<SponsorshipRequest>(`/api/manager/requests/${id}/approve`, { method: "POST", body: JSON.stringify({ remarks }) }, token),
  managerReject: (token: string, id: string, remarks: string) =>
    request<SponsorshipRequest>(`/api/manager/requests/${id}/reject`, { method: "POST", body: JSON.stringify({ remarks }) }, token),
  ...
  financeApprove: (token: string, id: string, remarks: string) =>
    request<SponsorshipRequest>(`/api/finance/requests/${id}/approve`, { method: "POST", body: JSON.stringify({ remarks }) }, token),
  financeReject: (token: string, id: string, remarks: string) =>
    request<SponsorshipRequest>(`/api/finance/requests/${id}/reject`, { method: "POST", body: JSON.stringify({ remarks }) }, token),
  adminRequests: (token: string) => request<SponsorshipRequest[]>("/api/admin/requests", {}, token),
  requestHistory: (token: string, id: string) => request<WorkflowHistory[]>(`/api/admin/requests/${id}/history`, {}, token),
  sponsorshipTypes: (token: string) => request<SponsorshipType[]>("/api/sponsorship-types", {}, token),
  adminSponsorshipTypes: (token: string) => request<SponsorshipType[]>("/api/admin/sponsorship-types", {}, token),
  createType: (token: string, name: string) =>
    request<SponsorshipType>("/api/admin/sponsorship-types", { method: "POST", body: JSON.stringify({ name, isActive: true }) }, token),
  updateType: (token: string, type: SponsorshipType) =>
    request<SponsorshipType>(`/api/admin/sponsorship-types/${type.id}`, { method: "PUT", body: JSON.stringify(type) }, token),
